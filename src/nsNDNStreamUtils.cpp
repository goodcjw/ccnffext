...
class nsNDNInputStreamReadyEvent : public nsIRunnable
                                 , public nsIInputStreamCallback
{
public:
  NS_DECL_ISUPPORTS

  nsNDNInputStreamReadyEvent(nsIInputStreamCallback *callback,
 ...
 private:
  ~nsNDNInputStreamReadyEvent()
  {
    ...
    if (NS_FAILED(rv) || !val) {
      ...
      NDN_NewInputStreamReadyEvent(getter_AddRefs(event), mCallback,
                                  mTarget);
      mCallback = 0;
      if (event) {
...
NS_IMPL_THREADSAFE_ISUPPORTS2(nsNDNInputStreamReadyEvent,
                              nsIRunnable,
...
public:
  ...
  nsresult Start(nsIInputStream *source,
                 ...
                 nsIEventTarget *target,
                 nsNDNAsyncCopyCallbackFun callback,
                 void *closure,
                 PRUint32 chunksize,
                 bool closeSource,
                 bool closeSink)
 ...
 protected:
  ...
  nsCOMPtr<nsIEventTarget>       mTarget;
  Mutex                          mLock;
  nsNDNAsyncCopyCallbackFun      mCallback;
  void                          *mClosure;
  PRUint32                       mChunkSize;
...
nsresult
NDN_NewInputStreamReadyEvent(nsIInputStreamCallback **event,
                             nsIInputStreamCallback *callback,
                             nsIEventTarget *target)
{
  NS_ASSERTION(callback, "null callback");
  NS_ASSERTION(target, "null target");
  nsNDNInputStreamReadyEvent *ev
      = new nsNDNInputStreamReadyEvent(callback, target);
  if (!ev)
...
nsresult
NDN_AsyncCopy(nsIInputStream            *source,
              nsIOutputStream           *sink,
              nsIEventTarget            *target,
              nsNDNAsyncCopyMode         mode,
              PRUint32                   chunkSize,
              nsNDNAsyncCopyCallbackFun  callback,
              void                      *closure,
              bool                       closeSource,
              ...
              nsISupports              **aCopierCtx)
{
  ...
  if (mode == NDN_ASYNCCOPY_VIA_READSEGMENTS)
...
NS_METHOD
NDN_CopySegmentToBuffer(nsIInputStream *inStr,
                        void *closure,
                        const char *buffer,
                        PRUint32 offset,
                        PRUint32 count,
...
NS_METHOD
NDN_WriteSegmentThunk(nsIInputStream *inStr,
                      void *closure,
                      const char *buffer,
                      PRUint32 offset,
                      PRUint32 count,
                      PRUint32 *countWritten)
{
  nsNDNWriteSegmentThunk *thunk = static_cast<nsNDNWriteSegmentThunk *>(closure);
